...
class EmergencyRefillTask(
    ...
    private fun placeShulker() {
        ...
        if (timeoutTimer.passedMs(3000L)) {
            if (!triedAlternative && placePos2 != null && placePos2 != target) {
                AgentUtils.sendMessage("§e在位置1放置迫降补给潜影盒超时，尝试在备用位置2放置...")
    ...
    private fun transferFireworks() {
        ...
        if (emptySlots <= 2) {
            AgentUtils.sendMessage("背包空间不足(仅剩$emptySlots 个空位)，容纳潜影盒与鞘翅保护，终止补给")
            phase = Phase.CLOSE_SHULKER
            setCooldown(0)
            return
        }
    ...
    private fun fail(message: String) {
        ...
        if (!finished) {
            ...
            minecraft.connection?.connection?.disconnect(Component.literal("[EmergencyRefill] $message"))
        }
    }
