...
class FlyToTakeOffTask(agent: ANAgent) : AITask(agent) {
    ...
    private fun prepare() {
        ...
        if (player.getItemBySlot(EquipmentSlot.CHEST).item != Items.ELYTRA) {
            ...
            completeModule("No Elytra equipped")
            finished = true
            return
        }
        if (!AgentUtils.hasFirework()) {
            AgentUtils.sendMessage("No Fireworks")
            completeModule("No Fireworks")
            finished = true
            return
        }
    ...
    private fun activate() {
        ...
        if (ticks > 60) {
            ...
            completeModule("Takeoff Timeout")
            finished = true
        }
    }
    ...
    private fun stabilize() {
        val player = player ?: return
        if (player.isFallFlying) {
            agent.scheduler.push(FlyToGlideTask(agent))
            finished = true
            return
        }
    ...
    private fun completeModule(message: String) {
        (agent.module as? ANFlyTo)?.complete(message)
    }
