...
class ThemeGroupElement(
    private val setting: ANSetting<ThemeGroupSetting>
    ...
    override fun render(context: ANGuiRenderContext, mouseX: Int, mouseY: Int, deltaTicks: Float) {
        val themes = ANConfigManager.themeNames()
        val currentName = ANConfigManager.currentThemeName()
        setting.value.setTheme_Name(currentName)

        height = dynamicHeight(themes.size)

        for (row in 0 until visibleRows(themes.size)) {
            val rowY = y + row * (ROW_HEIGHT + ROW_GAP)
            if (row < themes.size && row < ROWS - 1) {
                renderThemeRow(context, mouseX, mouseY, rowY, themes[row], currentName)
            } else {
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        ...
        val themes = ANConfigManager.themeNames()
        if (row < themes.size && row < ROWS - 1) {
            ANConfigManager.chooseTheme(themes[row])
            inputMode = false
            inputText = ""
            inputRow = -1
        } else {
    ...
    override fun keyPressed(key: Int, scanCode: Int, modifiers: Int): Boolean {
        ...
        when (key) {
            ...
            GLFW.GLFW_KEY_ENTER -> {
                ANConfigManager.createTheme(inputText)
                cancelInput()
            }
    ...
    private fun renderThemeRow(context: ANGuiRenderContext, mouseX: Int, mouseY: Int, rowY: Float, name: String, currentName: String) {
    ...
    private fun renderAddRow(context: ANGuiRenderContext, mouseX: Int, mouseY: Int, row: Int, rowY: Float) {
        ...
        if (editing) {
            val text = inputText.ifEmpty { "Theme name" }
